...
OUTPUT_FILE = 'prp_mfea_trajectory.csv'
...
def setup_mapf_tasks():
    ...
    for i in range(len(df_cleaned)):
        ...
        mapf_tasks.append({
            ...
            'start': start_grid,
            'end': end_grid,
            # CRITICAL ADDITION: Heuristic for MFEA - Distance to goal
            'dist_heuristic': abs(start_grid[0] - end_grid[0]) + abs(start_grid[1] - end_grid[1]) + abs(
                start_grid[2] - end_grid[2])
        })

    # CRITICAL FIX 1: Sort tasks by Heuristic (MFEA logic)
    # Give priority to agents that need to travel the longest distance first.
    mapf_tasks.sort(key=lambda x: x['dist_heuristic'], reverse=True)
...
# --- 3. ALGORITHMS IMPLEMENTATION (PRP with MFEA Priority) ---

def run_prp_planner(tasks, dims):
    """
    Implements the Priority-Based Planner (PRP) using MFEA-like dynamic priority.
    """
    start_time = time.time()

    paths = {}
    occupied_cells = set()  # Store (x, y, z, t) tuples

    print(f"Running PRP Planner (MFEA Priority) for {len(tasks)} drones...")

    for task in tasks:
        drone_id = task['drone_id']

        # 1. Define Constraints from previously planned paths
        path_constraint = set()
        for path in paths.values():
            for t, node in enumerate(path):
                # Vertex conflict: node occupied at time t
                path_constraint.add((*node, t))

        # 2. Run A* search with constraints
        # NOTE: The wait action is needed in a_star_searcher.py for this to work robustly.
        path = a_star_search(
            start_node=task['start'],
            goal_node=task['end'],
            dims=dims,
            occupied_cells=set(),  # No static obstacles
            path_constraint=path_constraint,
            max_time_steps=MAX_STEPS_ALLOWED
        )

        if path is None:
            # If path still fails, it means MFEA logic didn't help.
            print(f"PRP (MFEA) FAILED to find a path for Drone {drone_id}!")
        ...
        for t, node in enumerate(path):
            ...
            occupied_cells.add((node[0], node[1], node[2], t))
    ...
    print("PRP (MFEA) execution successful.")
...
if __name__ == '__main__':
    print("--- Starting Trajectory Planning (PRP with MFEA Priority) ---")
